...
export class EnderecoService {
  ...
  async internalEnderecoCreateOrUpdate(
    id: LadesaTypings.Endereco["id"] | null,
    payload: LadesaTypings.EnderecoInput
  ) {
    const enderecoInputValidator =
      await makeValidatorForEntity<LadesaTypings.EnderecoInput>(
        LadesaTypings.Tokens.EnderecoInputView
      );
  ...
  async findById(
    ...
    dto: LadesaTypings.EnderecoFindOneInputView,
    selection?: string[] | boolean
  ): Promise<LadesaTypings.EnderecoFindOneResultView | null> {
    ...
    qb.select([]);
    QbEfficientLoad(
      LadesaTypings.Tokens.EnderecoFindOneResultView,
      qb,
      aliasEndereco,
      selection
    );
